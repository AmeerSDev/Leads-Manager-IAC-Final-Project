# Leads Manager (IT Asset Management)
Make your IT Asset Management process simple and controlled. Leads Manager is a complete web application for tracking computer equipment, software licenses, and accessories in an organization. The application provides a simple and controlled way to manage IT assets, assign items to users, and track inventory status.
...
These steps mirror what we validated up to “perfect the DNS work”. Do this on a clean clone.

### 0) Clone and prerequisites
- `git clone https://github.com/AmeerSDev/Leads-Manager-IAC-Final-Project.git && cd Leads-Manager-IAC-Final-Project`
- Install: Docker, Terraform ≥1.6, kubectl, helm, AWS CLI, Python 3.11+ with pytest.
- Never commit real AWS/Docker credentials. Keep them local or as GitHub Secrets.
...
curl -X POST -H "Content-Type: application/json" -d '{"name":"Test","last_name":"User","deal-id":42,"is_active":true,"est_value":1234}' http://localhost:5000/leads
curl -X POST http://localhost:5000/leads/close/42
```
Stop with Ctrl+C; data persists in `./tmp-data`.

### 2) Terraform plan/apply (AWS sandbox)
Edit `iac/terraform/terraform.tfvars` with sandbox credentials/placeholders (do not commit real secrets):
```
...
terraform output
```
Note the outputs: CONTROL-PLANE-PUBLIC-IP, WORKER-1-PUBLIC-IP, WORKER-2-PUBLIC-IP, ALB-DNS-NAME.

### 3) Join workers (if they didn’t auto-join)
On control plane:
```
...
### 4) Deploy app with Helm (on control plane)
```
cd /home/ubuntu/helm
...
### 5) Verify end-to-end via ALB
- From anywhere: `curl http://<ALB-DNS-NAME>/health` and `http://<ALB-DNS-NAME>/leads`
- If you see 502, ensure workers are joined and target group is healthy on port 32000.

### 6) CI/CD (optional)
- Set GitHub Secrets: AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY / AWS_SESSION_TOKEN / AWS_REGION / AWS_AMI_ID / AWS_INSTANCE_TYPE / DOCKER_USERNAME / DOCKER_PASSWORD.
- Push to `main` or use “Run workflow” to trigger `.github/workflows/cicd.yml` (tests → build/push image → Terraform → join workers via SSH → Ansible NFS → Helm deploy).
...
For Application Deployment please use separate [Application User Guide](https://github.com/dcoacher/Leads Manager/blob/main/USER-GUIDE.md).

## 7. License
[![GPLv3 License](https://img.shields.io/badge/License-GPL%20v3-yellow.svg)](https://github.com/dcoacher/Leads Manager/blob/main/LICENSE)
