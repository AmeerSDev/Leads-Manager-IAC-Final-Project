...
resource "tls_private_key" "LEADS_MANAGER_KP" {
  algorithm = "RSA"
  rsa_bits  = 4096
}
...
resource "local_sensitive_file" "private_key" {
  content         = tls_private_key.LEADS_MANAGER_KP.private_key_pem
  filename        = "${path.module}/KP.pem"
  file_permission = "0600"
}

# Create AWS key pair from the generated public key
resource "aws_key_pair" "LEADS_MANAGER_KP" {
  provider = aws.North-Virginia
  key_name   = "leads-manager-keypair"
  public_key = tls_private_key.LEADS_MANAGER_KP.public_key_openssh

  tags = {
    Name = "leads-manager-keypair"
  }
}
...
resource "terraform_data" "KP-Copy" {
  ...
  triggers_replace = {
    key_checksum = sha256(tls_private_key.LEADS_MANAGER_KP.private_key_pem)
  ...
  connection {
    type        = "ssh"
    user        = "ubuntu"
    private_key = tls_private_key.LEADS_MANAGER_KP.private_key_pem
