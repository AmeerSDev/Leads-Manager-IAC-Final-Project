...
apiVersion: v2
name: leads-manager
description: Leads Manager Flask web application
type: application
version: 1.0.0
appVersion: "1.0"
EOF
...
replicaCount: 2
image:
  repository: docker.io/${docker_username}/leads-manager
  tag: "latest"
  pullPolicy: Always
service:
  type: NodePort
...
if command -v helm &> /dev/null; then
  if helm list -n default | grep -q "leads-manager"; then
    if [ -n "$IMAGE_REPO" ] && [ -n "$IMAGE_TAG" ]; then
      helm upgrade --install leads-manager . \
        --values values.yaml \
      ...
      helm upgrade --install leads-manager . --values values.yaml --timeout 5m --wait=false
    fi
  else
    if kubectl get pvc leads-manager-pvc -n default &>/dev/null; then
      MANAGED_BY=$(kubectl get pvc leads-manager-pvc -n default -o jsonpath='{.metadata.labels.app\.kubernetes\.io/managed-by}' 2>/dev/null || echo "")
      if [ "$MANAGED_BY" != "Helm" ]; then
        if kubectl get deployment leads-manager -n default &>/dev/null; then
          kubectl scale deployment leads-manager -n default --replicas=0 || true
          sleep 5
        fi
        kubectl delete pvc leads-manager-pvc -n default --wait=false || true
        sleep 3
      fi
    fi
    if kubectl get deployment leads-manager -n default &>/dev/null; then
      MANAGED_BY=$(kubectl get deployment leads-manager -n default -o jsonpath='{.metadata.labels.app\.kubernetes\.io/managed-by}' 2>/dev/null || echo "")
      if [ "$MANAGED_BY" != "Helm" ]; then
        kubectl delete deployment leads-manager -n default --wait=false || true
      fi
    fi
    if kubectl get svc leads-manager -n default &>/dev/null; then
      MANAGED_BY=$(kubectl get svc leads-manager -n default -o jsonpath='{.metadata.labels.app\.kubernetes\.io/managed-by}' 2>/dev/null || echo "")
      if [ "$MANAGED_BY" != "Helm" ]; then
        kubectl delete svc leads-manager -n default --wait=false || true
      fi
    fi
    sleep 5
    if [ -n "$IMAGE_REPO" ] && [ -n "$IMAGE_TAG" ]; then
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
  ...
  if kubectl wait --for=condition=available deployment/leads-manager --timeout=15s &>/dev/null 2>&1; then
    break
  fi
  sleep 15
  ELAPSED=$((ELAPSED + 15))
